...
//        return Auth::check() || Auth::guard("admin")->check();
    ...
    public function rules()
    {
        ...
        $rules = [
            ...
            "email" => ["string", "email", "nullable", "exists:clients,email"],
            "dateFrom" => ["string", "nullable", "date_format:Y-m-d H:i:s", "before:dateTo"],
            "dateTo" => ["string", "nullable", "date_format:Y-m-d H:i:s"],
        ];

        foreach ($formRequests as $source) {
            $rules = array_merge(
                $rules,
                (new $source)->rules("clients")
            );
        }
